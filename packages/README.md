...
它们不复制 React、Auth.js、Prisma 或 CopilotKit 的源码，而是提供完整软件常见底层边界：核心类型、认证、权限、安全、数据、API 和审计。后续可以把内存仓库替换成 PostgreSQL/Prisma，把 Session 替换成真实 Cookie/JWT，把 API 挂到 Node.js、Next.js 或 Fastify。
...
- `data` 定义模型和 Repository，不关心 HTTP。
